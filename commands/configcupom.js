...
<:Dinheiro:1101308742672322660>・Porcentagem: ${db.get(`${adb}.desconto`)}%`)
        ...
        interação.on("collect", async (interaction) => {
         ...
         if (interaction.customId === "qtdcupom") {
             ...
             msg.channel.send("<a:Load:1101670678320652318> | Qual a nova quantidade de usos?").then(msg => {
               ...
               collector.on("collect", message => {
                 message.delete()
                 if (isNaN(message.content)) return msg.edit("<a:emoji_83:1101311855852527676> | Não coloque nenhum caractere especial além de números.")
         ...
         if (interaction.customId === "mincupom") {
             interaction.deferUpdate();
             msg.channel.send("<a:Load:1101670678320652318> | Qual o novo mínimo para uso em reais?").then(msg => {
               const filter = m => m.author.id === interaction.user.id;
               const collector = msg.channel.createMessageCollector({ filter, max: 1 });
               collector.on("collect", message => {
                 message.delete()
...
➕・Quantidade: ${db.get(`${adb}.quantidade`)}
➖・Mínimo: ${db.get(`${adb}.minimo`)} Reais
<a:svgiveway:1101644989739565118>・Desconto: ${db.get(`${adb}.desconto`)}%`)
             .setThumbnail(client.user.displayAvatarURL())
